...
- Prisma runtime-history migration verification is now real, not just scaffolded: `npm exec prisma migrate deploy -- --schema .\prisma\schema.prisma` reports no pending migrations, and `npm run runtime-history:check` confirms both `OrchestrationRuntimeSnapshot` and `OrchestrationRuntimeEvent` tables plus required columns exist in the local `aifut` database.
...
- Latest checked local HEAD: `c35e4da` (`merge origin/main into main`).
