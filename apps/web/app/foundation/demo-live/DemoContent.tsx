...
import {
  API_BASE,
  getJson,
  type AdapterInterfaceRegistryResponse,
  type HealthResponse,
...
export default function DemoLiveContent() {
  const [data, setData] = useState<{
    health: HealthResponse | null;
    interfaces: AdapterInterfaceRegistryResponse | null;
    contractsNext: string[] | null;
    templates: unknown[] | null;
  } | null>(null);

  const loadData = useCallback(async () => {
    const [health, interfacesResponse, contractsResponse, templatesResponse] =
      await Promise.all([
    ...
    setData({
      health,
      interfaces: interfacesResponse?.adapterInterfaces ?? null,
      contractsNext: contractsResponse?.adapterContracts?.next ?? null,
      templates: templatesResponse?.templates?.templates ?? null,
    });
  }, []);
  ...
  const interfaces = data?.interfaces ?? null;
  const contractsNext = data?.contractsNext ?? null;
    ...
    >
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <h1 style={{ fontSize: 42, margin: "18px 0 12px" }}>
          Visible demo — governed adapter interfaces
        </h1>
        <p
          style={{
            color: "#c8d2ff",
        ...
        >
          This is the clearest current proof of the &quot;kernel above tools&quot;
          idea: AIFUT does not just list connectors. It defines normalized
          interfaces, activation policy, and runtime binding so apps, AI,
          workflows, and operator actions can be coordinated under one control
          plane.
        </p>
        ...
        >
          ...
          <LinkButton href={`${API_BASE}/connectors/adapter-interfaces`}>
            /connectors/adapter-interfaces
          </LinkButton>
          <LinkButton href={`${API_BASE}/connectors/adapter-contracts`}>
            /connectors/adapter-contracts
          </LinkButton>
          <LinkButton href={`${API_BASE}/connectors/templates`}>
        ...
        <section style={{ marginTop: 28 }}>
          <Panel title="Why this matters">
            <Bullet>
              Normal connectors only say what system can connect.
            </Bullet>
            <Bullet>
              Adapter interfaces say what business-safe request/response
              contract AIFUT expects.
            </Bullet>
            <Bullet>
              That creates the path for natural-language setup, governed AI
              execution, approval gates, and reusable runtime contracts.
            </Bullet>
          </Panel>
        </section>
            ...
            {records.map(
              (item: {
                ...
                <div key={item.key} style={cardStyle}>
                  ...
                  >
                    app: {item.appDefinitionKey} • connector:{" "}
                    {item.connectorKey}
                  </div>
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div>
                      {item.requestShape} → {item.responseShape}
                    </div>
                  </div>
                  <div style={{ marginTop: 12 }}>
          ...
          <Panel title="Roadmap hooks already exposed in API">
            <div style={{ display: "grid", gap: 8 }}>
              {(interfaces?.next ?? []).map((item: string) => (
                <div key={item} style={{ color: "#dfe6ff" }}>
                  • {item}
                </div>
              ))}
...
function Bullet({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ color: "#dfe6ff", lineHeight: 1.7 }}>• {children}</div>
  );
}

function Label({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ fontSize: 12, color: "#9fb0ff", marginBottom: 6 }}>
      {children}
    </div>
  );
}
