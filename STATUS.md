...
- `POST /orchestration/business-systems/runtime-binding-setup-preview` now reviews candidate runtime-binding setup values without activation. It returns `ready-for-operator-review` when required inputs are present, `blocked-pending-inputs` when required/valid inputs are missing, and always keeps `previewOnly: true`, `externalActionsAllowed: false`, and `activationAllowed: false`.
- Verification passed: targeted app/orchestration tests `92/92`, API build, full API Jest `26/26` suites and `374/374` tests, live ready setup-preview proof, live missing/invalid setup-preview proof, and API root discovery proof for the new endpoint.
...
- Runtime-binding setup preview now exposes bounded `operatorDecisionState` for operator review, with explicit allowed decisions, audit intent key, and preview-only activation boundary.
- Web HQ renders the operator decision status, allowed decision set, review gate, activation boundary, and audit intent alongside the existing runtime-binding setup review.
- Verification passed: targeted orchestration service spec `59/59`, full API Jest `26/26` suites and `377/377` tests, API build, root build, web typecheck, runtime-history check, local runtime verifier `ok: true`, live setup-preview proof with `awaiting-operator-decision`, and dashboard render `200` on local API/Web ports `3002`/`3000`.
